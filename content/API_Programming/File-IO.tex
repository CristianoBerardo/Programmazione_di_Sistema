...
\begin{description}
  ...
  \item \mintinline{rust}|read_to_string(buf: &mut String) -> Result<usize>| - continua a
        leggere fino all'EOF e riceve come parametro una \mintinline{rust}|&mut String|.
  \item \mintinline{rust}|read_exact(buf: &mut [u8]) -> Result<()>| - prova a leggere l'esatto
  ...
  \item \mintinline{rust}|take(limit: u64) -> Take<Self>|  - limita il numero di byte che possono essere letti da un reader
\end{description}
...
è l'alias di \mintinline{rust}|Result<T, io::Error>|
...
\begin{itemize}
  \item Alcune operazioni possono essere semplicemente ritentate
  \item Altre indicano fine input, permessi insufficienti o dati non validi
\end{itemize}
...
\begin{itemize}
  ...
  \item Usare BufReader per letture piccole/frequenti
  ...
  \item Usare OpenOptions per semantiche di apertura esplicite
