...
\begin{flushleft}
  ...
  \begin{minipage}{0.15\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/sdfrbn.png}
  \end{minipage}
\end{flushleft}

\subsection{Driver dei Dispositivi - I/O Controller}
...
Abbiamo system call al livello API: Open, close, read, write, seek... ma come implementiamo le loro funzioni? Mediante strutture \textbf{su disco} e \textbf{in memoria}.
...
\#indici/blocco = 4K/4B = 1K

file = 4GB = 4GB/4KB = 1M blocchi

1M indice nella tabella (un indice per ogni blocco)

tabella richiede 1K blocchi di 2ndo livello
1 blocco di primo livello

se file > 4GB serve un terzo livello.


Essendo che non sappiamo quanto è grande il file, quando parti (primi 10)
sono gratis e li metto nel FCB (inode) se cresce gioco l'indice ad un solo livello
ed ospito 1K blocchi.

\paragraph{}


I file piccoli sono veloci, quelli grandi sono più lenti
ma hai tanto da leggere e scrivere.
...
\begin{center}
  ...
  \begin{minipage}{0.55\textwidth}
    \begin{minipage}[h!]{0.47\textwidth}
      ...
      \captionof{figure}{Il Linux iNode}
    \end{minipage}
    \begin{minipage}[h!]{0.47\textwidth}
      \centering
      \includegraphics[width=1\linewidth]{content/OS_Internals/img/smgth.png}
      \captionof{figure}{UNIX UFS - 4 KB per blocco, indirizzi a 32 bit}
    \end{minipage}
  \end{minipage}
  % \end{figure}
\end{center}
...
\begin{flushleft}
  \begin{minipage}{0.65\linewidth}
    ...
    La open() cerca, mentre la read() si dimentica dei direttori e legge.
    Il processo A può avere aperto un file 5.
    Un'altro processo può avere un altro file 5 ma può essere un'altro file

    Se ogni file fosse accessibile esclusivamente per ogni processo
    (no shering) basterebbe la \textbf{tabella dei file aperti per processo}
    mentre la \textbf{tabella globale dei file aperti} tiene traccia dei lock
    sui vari file.
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/mxfhg.png}
  \end{minipage}
\end{flushleft}
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/dndsgvg.png}
  \end{minipage}
\end{flushleft}
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/dghgf.png}
  \end{minipage}
\end{flushleft}
...
Contigua, Collegata, Tabella di Allocazione dei File (FAT)
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/srymkh.png}
  \end{minipage}
\end{flushleft}
...
\begin{flushleft}
  \begin{minipage}{0.7\linewidth}
    \raggedright
    \subsection{Allocazione Collegata - Linked}
    ...
    \begin{itemize}
      ...
      \item Ogni blocco contiene un puntatore al blocco successivo (minimo overhead)
      ...
      \item Se perdi un puntatore perdi il resto del file/ dei file
    \end{itemize}
  ...
  \begin{minipage}{0.25\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/srfymjkgm.png}
  \end{minipage}
\end{flushleft}
...
Se vogliamo capire dove sta il blocco 1500 dividiamo per (512-1) dato il puntatore.
...
\subsection{Metodo di Allocazione File-Allocation Table FAT}

\begin{minipage}{0.75\textwidth}
  Scorporiamo i puntatori dai file e mantengo il pointer di 1 e il blocco 512.
  Due vettori di dati PARALLELI. Devo stare attento a non corrompere il vettore
  dei pointer.

  L'inizio del volume ha una tabella, indicizzata per numero di blocco. Simile a una lista collegata, ma più veloce su disco e memorizzabile in cache. L'allocazione di nuovi blocchi è semplice.

  Struttura che contiene solo il puntatore
\end{minipage}
\hfill\
\begin{minipage}{0.2\textwidth}
  \begin{center}
    \includegraphics[width=\linewidth]{content/OS_Internals/img/nddgndg.png}
  \end{center}
\end{minipage}

\vspace{1.5em} % Aggiunge un po' di respiro tra le due sottosezioni

\subsection{Metodo di Allocazione Indicizzata}

\begin{minipage}{0.75\textwidth}
  C'è un difetto delle liste: l'accesso diretto è O(n). Usando la FAT devo sempre
  scorrere il vettore dei puntatori anche se molto meglio di una lista collegata.

  Ogni file ha il proprio/i propri blocco/i indice con puntatori ai propri blocchi dati.
  Può esserci frammentazione interna, dipende quanto occupa il file.
  Filosofia simile alla page-table.


  Se ti serve più spazio:
  \begin{itemize}
    \item Metti più blocchi indice in lista
  \end{itemize}
\end{minipage}
\hfill
\begin{minipage}{0.2\textwidth}
  \begin{center}
    \includegraphics[width=\linewidth]{content/OS_Internals/img/fbfbf.png}
  \end{center}
\end{minipage}
...
\begin{itemize}
  \item L'allocazione contigua è ottima per accesso sequenziale e casuale, unico difetto c'è frammentazione
  \item L'allocazione collegata è buona per l'accesso sequenziale, non per quello casuale
  \item L'indicizzata è più complessa: per accedere a un singolo blocco potrebbe essere necessario leggere prima il blocco indice e poi il blocco dati.
\end{itemize}
...
\begin{center}
  \begin{minipage}{0.65\linewidth}
    ...
    \begin{tcolorbox}
      \textbf{Nota:} la scelta di che struttura usare per allocare file e della gestione dello spazio libero può essere diversa (inode per i file e lista continua per il free space)
    \end{tcolorbox}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/mngaz.png}
    \captionof{figure}{bit map}
  \end{minipage}
\end{center}
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.75\linewidth]{content/OS_Internals/img/andjfgt.png}
  \end{minipage}
\end{flushleft}
...
\subsection{TRIMing Unused Blocks}

Nelle SSD ci sono i blocchi liberi ma ancora da cancellare

\subsection{Efficiency and Performance of File Systems}

\begin{itemize}
  \item Come organizzi i file
  \item Come organizzi le directory
  \item Come organizzi lo spazio libero
  \item Come gestisci i metadati (file control block)
\end{itemize}

In questo contesto possiamo pensare a strutture dati a lunghezza fissa
o a lunghezza variabile.

Per migliorare le prestazioni bisogna fare attenzione a:
quanto è O(1) o O(n) ma quanto incide?

\begin{itemize}
  \item tenere dati e metadati vicini
  \item Buffer cache - non leggo sempre il disco ma lo tengo in memoria per blocchi usati frequentemente. Riduce le richieste al disco.
  \item Distinguere scritture sincrone e asincrone
  \item Read-ahead - lettura seq. da file con lettura blocco 3 leggo anche il 4 se sono vicini. Migliore accesso sequenziale
\end{itemize}
...
\begin{center}
  \begin{minipage}{0.65\linewidth}
    Il \textit{memory-mapped I/O} scrivi su file senza usare le read e write, il SO
    per essere efficiente usa una \textbf{page cache}.

    Quindi 2 accessi a file presente nell'immagine.
  \end{minipage}%
  \hfill
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.4\linewidth]{content/OS_Internals/images/2026-05-22-10-19-09.png}  \end{minipage}
\end{center}
...
\begin{center}
  \begin{minipage}{0.65\linewidth}
    ...
    \paragraph{Unified Buffer Cache }
    Per evitare di avere due copie e ottimizzazione a più livelli di cache si è passato ad avere un
    \textbf{unified buffer cache}, meno overhead.
  \end{minipage}%
  \hfill
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.4\linewidth]{content/OS_Internals/images/2026-05-22-10-24-56.png}
  \end{minipage}
\end{center}
...
Avere ridondanza sui file, come i dischi, posso usare i backup.
Ma tra un backup e l'altro si perde qualcosa. Si può usare quindi
il \textit{journal}.

\subsection{Log Structured File Systems}
Come agire se si rompe a metà strada? Mantine una copia della transazione
e prima di effetturare l'operazione salva cosa si voleva fare questo
si ottieme uno stato più consistente.

Una volta che sono nel log si considerano fatte.
La transazione dunque è come dire "voglio fare il bonifico" e poi in modo
asincrono verrà eseguita.
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.2\linewidth]{content/OS_Internals/img/dnfga.png}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.2\linewidth]{content/OS_Internals/img/zndfg.png}
...
Vogliamo evitare che un thread sia in uno stato zombiee, ovvero che nessuno
ha collezionato il risultato del thread e i suoi metadati.
Si vuole implementare waitpid, attesa di fine processo.
Inizialmente crealo lato kernel dato che non serve il PID,
perché il kernel ha accesso alla strutture e metadati.

proc.h e in struct proc aggiungere p\_id e p\_status e un semaforo (o cvar con boolean, cv e un lock).


\begin{tcolorbox}
  \textbf{Nota:} la soluzione propsta per cvar non include l'uso del bool. Quindi in proc.h è aggiunto bool p\_terminated e poi devi implementarlo.

\end{tcolorbox}


proc.c proc\_create criamo il semaforo o la cvar e poi le distruggo
quando uso la proc\_destroy

successivamente quando abbiamo creato ciò creiamo la proc\_wait

un attesa se è un semaforo o un attesa e un while  e poi
prendo i dati di ritorno.

Questa era la prima parte del kernel.

\paragraph{Ora modifichiamo la sysexit}
Quella precedente nel lab 2 distruggeva la struct proc (che ora è nella proc
wait) mentre ora vogliamo collezionare i dati.
proc\_syscall.c

poi deve fare un thread\_exit, ma attenzione che
se il padre viene svegliato tropo presto potrebbe essere
che voglia distriigere un figlio ancora in esexuioen.
Per risolvere al posto di scriverla della proc\_destry la faccio nella
proc\_syscall.c (V->p\_sem) e dobbiamo intrdurrre in thread.c in thread\_exit
deve essere chiamata anche quando non cis ono processi in esecuzione.


if(cur->t\_prc != null) {
    proc\_destroy(cur->t\_prc);
    cur->t\_prc = null;
  }else{
