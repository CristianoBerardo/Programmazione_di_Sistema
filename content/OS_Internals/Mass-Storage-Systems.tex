...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/yys.png}
  \end{minipage}
\end{flushleft}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/Immagine 2024-05-22 183104.png}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/Immagine 2024-05-22 183628.png}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/Immagine 2024-05-22 184919.png}
\end{figure}
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.7\linewidth]{content/OS_Internals/img/fhmgmfgzh.png}
    \captionof{figure}{Avvio dallo storage secondario in Windows}
  \end{minipage}
\end{flushleft}
...
\begin{flushleft}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.3\linewidth]{content/OS_Internals/img/amrjkyt.png}
    \captionof{figure}{Strutture dati per lo swapping nei sistemi Linux}
  \end{minipage}
\end{flushleft}
...
\begin{center}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.75\linewidth]{content/OS_Internals/img/tdmng.png}
    \captionof{figure}{NAS}
  \end{minipage}
\end{center}
...
\begin{center}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.75\linewidth]{content/OS_Internals/images/2026-03-22-17-19-31.png}
  \end{minipage}
\end{center}
...
\begin{itemize}
  ...
  \item \textbf{Tempo medio di riparazione - MTTR}: periodo di esposizione durante il quale un altro guasto potrebbe causare perdita di dati
  \item \textbf{Tempo medio alla perdita di dati - MTTF}: basato sui fattori precedenti
\end{itemize}


Se i dischi speculari si guastano indipendentemente, si consideri un disco con 100.000 (MTTF) ore di tempo medio al guasto e 10 ore di tempo medio di riparazione (MTTR). Il tempo medio alla perdita di dati è:
...
\begin{flushleft}
  \begin{minipage}{0.65\linewidth}
    ...
    \begin{itemize}
      \item RAID 0: striping senza ridondanza. La probabilità che si guastino 4 dispositivi assieme è molto molto bassa.
      \item RAID 1: mirroring, duplica i dati, stesso metodo utilizzato nell'aviazione.
    \end{itemize}
...
\begin{center}
  \begin{minipage}{0.75\linewidth}
    \begin{minipage}[t]{0.47\linewidth}
      \centering
      \includegraphics[width=0.3\linewidth]{content/OS_Internals/images/2026-03-22-17-32-18.png}
    ...
    \begin{minipage}[t]{0.47\linewidth}
      \centering
      \includegraphics[width=0.3\linewidth]{content/OS_Internals/images/2026-03-22-17-32-56.png}
...
%     \raggedright
    ...
    \begin{itemize}
      \item File di Dati: Numerico, Carattere, Binario
...
%   \begin{minipage}{0.3\linewidth}
%     \centering
%     \includegraphics[width=0.7\linewidth]{content/OS_Internals/img/fgnfnfg.png}
%     \captionof{figure}{Tipi di file}
%   \end{minipage}
% \end{flushleft}
...
\begin{itemize}
  \item Creare
  \item Scrivere - alla posizione del puntatore di scrittura
  \item Leggere - alla posizione del puntatore di lettura
  \item Riposizionare all'interno del file - seek
  \item Eliminare
  \item Truncare
  \item Aprire (Fi) - cercare la struttura della directory sul disco per la voce Fi, e spostare il contenuto della voce in memoria
  \item Chiudere (Fi) - spostare il contenuto della voce Fi in memoria alla struttura della directory sul disco
\end{itemize}

\subsection{File Aperti}

Sono necessari diversi dati per gestire i file aperti:
\begin{itemize}
  \item Tabella dei file aperti: traccia i file aperti
  ...
  \item Contatore di aperture del file: contatore del numero di volte in cui un file è aperto, per consentire la rimozione dei dati dalla tabella dei file aperti quando l'ultimo processo lo chiude
...
\subsection{Open file locking}

Provided by some operating systems and ﬁle systems. Similar to reader-writer locks

\begin{itemize}
  \item Shared lock similar to reader lock -- several processes can acquire concurrently
  \item Exclusive lock similar to writer lock
\end{itemize}

Mediates access to a ﬁle. Mandatory or advisory:

\begin{itemize}
  \item  Mandatory - access is denied depending on locks held and requested
  \item  Advisory - processes can ﬁnd status of locks and decide what to do
\end{itemize}
...
Operazioni: \texttt{read next}, \texttt{write next}, \texttt{reset}; nessuna lettura dopo l'ultima scrittura (rewrite) e nessuna operazione di seek.


\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/andfg.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/dbfs.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/img/mhfg.png}
  \caption{Accesso sequenziale vs diretto}
\end{figure}
...
\begin{flushleft}
  \begin{minipage}{0.65\linewidth}
    ...
    \begin{tcolorbox}
      \textbf{Nota:} Questa è una struttura data ad alto livello, più avanti ne vedremo una di più basso livello.
    \end{tcolorbox}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/dfb.png}
  \end{minipage}
\end{flushleft}
...
\begin{center}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/mfh.png}
    \captionof{figure}{File mappati in memoria}
  \end{minipage}
\end{center}
...
Una collezione di nodi contenenti informazioni su tutti i file, ed alcuni
sui attributi come data di creazione, dimensioni, etc.


\begin{figure}[h!]
  \centering
  \includegraphics[width=0.15\linewidth]{content/OS_Internals/img/jhg.png}
\end{figure}
...
I direttori fanno parte dei metadati.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.35\linewidth]{content/OS_Internals/img/dbgnf.png}
\end{figure}
...
\begin{itemize}
  ...
  \item Denominazione - comoda per gli utenti: due utenti possono avere lo stesso nome per file diversi; lo stesso file può avere più nomi diversi per essere compatibili con programmi diversi
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.3\linewidth]{content/OS_Internals/img/dfgnb.png}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.3\linewidth]{content/OS_Internals/img/fng.png}
...
\begin{center}
  \centering % Centra l'intero contenuto della figura

  \begin{minipage}[c]{0.3\textwidth}
    \centering
    \includegraphics[width=1\linewidth]{content/OS_Internals/img/dfsbv.png}
  \end{minipage}
  \hfill % Aggiunge spazio elastico tra le due minipage
  \begin{minipage}[c]{0.6\textwidth}
    \centering
    \renewcommand{\arraystretch}{1.5}
    % Rimosso \begin{table}, usiamo direttamente tabular
    \begin{tabular}{c c}
      \toprule
      \textbf{Pro}                  & \textbf{Contro}                       \\
      \midrule
      Concetto di current directory & Inefficiente                          \\
      Raggruppamento                & Nessuna condivisione di file          \\
...
Traduci in italiano:
Nome del percorso assoluto o relativo.
Creare un nuovo file viene fatto nella directory corrente.
Eliminare un file
\verb|rm <file-name>|.
Creare una nuova sottodirectory viene fatto nella directory corrente
\verb|mkdir <dir-name>|
...
\begin{flushleft}
  \begin{minipage}{0.65\linewidth}
    ...
    \begin{itemize}
      \item \textbf{Backpointer - elimina tutto}, in modo da poter eliminare tutti i puntatori alla cartella eliminata; (records di dimensione variabile sono un problema.) Così lo cancelli da una parte e lui di conseguenza sale e cancella i direttori.
      \item \textbf{Backpointer} usando un'organizzazione a catena (daisy chain)
      \item \textbf{entry-hold-count - elimina il puntatore} Soluzione con contatore di riferimenti, elimino solo il puntatore al file e mantengo un contatore ai file. Il file viene cancellato solo se il contatore raggiunge zero.
    \end{itemize}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=\linewidth]{content/OS_Internals/img/fdbg.png}
  \end{minipage}
\end{flushleft}


\begin{figure}[h!]
  % Utilizziamo [t] per allineare le minipage in alto (top)
  \begin{minipage}[t]{0.48\linewidth}
    \raggedright % Garantisce l'allineamento a sinistra del testo
    ...
    \vspace{10pt} % Spazio tra testo e immagine
    {\centering
      \includegraphics[width=0.7\linewidth]{content/OS_Internals/img/dngf.png}
      \par}
  \end{minipage}%
  \hfill
  \begin{minipage}[t]{0.48\linewidth}
    ...
    Un file system deve essere montato prima di poter essere accessibile. Un file system non montato è un’entità separata che non fa parte dell’albero logico principale.

    \vspace{10pt}
    {\centering
      \includegraphics[width=0.7\linewidth]{content/OS_Internals/images/2026-03-22-17-53-56.png}
      \par}
  \end{minipage}
\end{figure}
...
La condivisione di file su sistemi multi-utente è desiderabile.
La condivisione può essere effettuata attraverso uno schema di protezione.
Su sistemi distribuiti, i file possono essere condivisi attraverso una rete.
Il Network File System (NFS) è un metodo comune di condivisione di file distribuita.

Se il sistema è multi-utente:

\begin{itemize}
  \item \textbf{User IDs}: Gli ID utente identificano gli utenti, consentendo che i permessi e le protezioni siano per utente.
  \item \textbf{Group IDs}: Gli ID gruppo consentono agli utenti di essere in gruppi, permettendo diritti di accesso per gruppo.
  \item \textbf{Owner of a file / directory}: Il proprietario di un file o directory.
  \item \textbf{Group of a file / directory}: Il gruppo a cui appartiene un file o directory.
\end{itemize}

\subsection{ Condivisione di File - Sistemi Remoti}

%  Uses networking to allow file system access between systems.
Utilizzare la rete per consentire l'accesso al file system tra sistemi.
...
\begin{itemize}
  \item Manualmente tramite programmi come FTP
  \item Automaticamente, in modo trasparente utilizzando file system distribuiti
  \item Semi-automaticamente tramite il World Wide Web
\end{itemize}

Il modello client-server consente ai client di montare file system remoti dai server:

\begin{itemize}
  \item Il server può servire più client
  \item L'identificazione del client e dell'utente sul client è insicura o complicata
  \item NFS è il protocollo standard UNIX per la condivisione di file in modalità client-server
  \item CIFS è il protocollo standard di Windows
  \item Le chiamate standard ai file del sistema operativo vengono tradotte in chiamate remote
\end{itemize}

I Sistemi Informativi Distribuiti (servizi di naming distribuiti) come LDAP, DNS, NIS, Active Directory implementano un accesso unificato alle informazioni necessarie per l'elaborazione remota


\subsection{Condivisione di File - Modalità di Guasto}

Tutti i file system presentano modalità di guasto, ad esempio la corruzione delle strutture delle directory o di altri dati non utente, chiamati metadati.
I file system remoti aggiungono nuove modalità di guasto, dovute a malfunzionamenti della rete o del server.
Il ripristino dai guasti può coinvolgere informazioni di stato relative allo status di ciascuna richiesta remota.

I protocolli stateless (senza stato) come NFS v3 includono tutte le informazioni in ogni singola richiesta, consentendo un ripristino semplice ma una minore sicurezza.


% \subsection{Condivisione di File - Semantica di Coerenza ()}

% Specifica come più utenti debbano accedere contemporaneamente a un file condiviso.
% Tendono a essere meno complesse a causa dell'I/O su disco e della latenza di rete (per i file system remoti). L'Andrew File System (AFS) ha implementato una complessa semantica per la condivisione remota dei file. Il file system Unix (UFS) implementa: ! Le scritture su un file aperto sono immediatamente visibili agli altri utenti dello stesso file aperto ! Condivisione del puntatore del file per consentire a più utenti di leggere e scrivere contemporaneamente. AFS ha una semantica di sessione ! Le scritture sono visibili solo alle sessioni che iniziano dopo la chiusura del file
...
\subsection{Liste di Accesso e Gruppi}
Modalità di accesso: lettura (read), scrittura (write), esecuzione (execute). Tre classi di utenti su Unix / Linux:

\begin{itemize}
  \item Accesso del proprietario (Owner) 7 $\to$ RWX (111)
  \item Accesso del gruppo (Group) 6 $\to$ RW- (110)
  \item Accesso pubblico (Public) 1 $\to$ --X (001)
\end{itemize}
...
\begin{center}
  ...
  \begin{minipage}{0.3\linewidth}
    \centering
    \includegraphics[width=0.6\linewidth]{content/OS_Internals/images/2026-03-22-18-06-47.png}
  \end{minipage}
\end{center}
