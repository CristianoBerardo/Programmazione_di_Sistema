...
\subsection{Passi nel richiedere Paging - Worse Case}
...
È basato sull'intervallo $\tau$ della prima page fault.


\begin{itemize}
  \item Se $\tau < c$ se il page fault è più grande di quella desiderata, aggiungi un frame al Resident Set del processo
  \item Se $\tau \geq c$ se la frequenza dei page fault è OK, rimuovi un frame dal Resident Set del processo con bit a 0.
\end{itemize}

poi pulisci, setta a 0, il bit di riferimento di tutte le altre pagine nel Resident Set.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\linewidth]{images/OS_internals/Main-memory/MM-esempio12.png}
  \caption{Esempio di frequenza di page fault con C = 3\footnote{In rosso le vittime. In Verde la pagina referenziata}}
\end{figure}

\section{Working Sets and Page Fault Rates}
Vi è una relazione diretta tra il working set di un processo e il suo tasso di page fault.
Il working set cambia nel tempo.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=1\linewidth]{images/OS_internals/Main-memory/MM-esempio13.png}
\end{figure}
...
Parte della memoria del kernel deve essere contigua, vedi I/O device.

\section{Buddy System}

Alloca memoria da segmenti di dimensioni fisse costituiti da pagine fisicamente
contigue, solitamente allocate usando dimensioni di potenza di 2:

\begin{itemize}
  \item Soddisfa le richieste in unità di dimensioni potenza di 2
  \item La richiesta viene arrotondata alla potenza di 2 più vicina
...
\textbf{Advantage}: quickly coalesce unused chunks into larger chunk.\newline

\noindent\textbf{Disadvantage}: fragmentation
...
\begin{center}
  ...
  \begin{minipage}{1\linewidth}
    ...
    \begin{minipage}{0.65\linewidth}
      ...
      \paragraph{Example: } assume 256KB chunk available, kernel requests 21KB.
      Split into $A_L$ and $A_R$ of 128KB each. One further divided into $B_L$ and $B_R$ of
      64KB. One further into $_CL$ and $C_R$ of 32KB each - one used to
      satisfy request.
    \end{minipage}% <-- IMPORTANTE: il simbolo % qui evita spazi bianchi extra
    \hfill
    \begin{minipage}{0.3\linewidth}
...
\begin{center}
  ...
  \begin{minipage}{1\linewidth}
    % Primo blocco: 50% dello spazio per il codice
    \begin{minipage}{0.65\linewidth}
      Alternate strategy use \textbf{slabs} - is one or more physically contiguous pages -
      and \textbf{Cache} - consists of one or more slabs.


      Single cache for each unique kernel data structure. Each cache ﬁlled
      with objects - instantiations of the data structure.
      When cache created, filled with objects marked as \texttt{free}, otherwise
      when structures stored, objects marked as \texttt{used}.

      If slab is full of used objects, next object allocated from empty
      slab. If no empty slabs, new slab allocated.

      Beneﬁts include no fragmentation, fast memory request satisfaction
...
\paragraph{Example - Slab Allocator in Linux}

For example process descriptor is of type \verb|struct task_struct| with
approximately 1.7KB of memory.

New task $\to$ allocate new structure from cache. Will use existing free
\verb|struct task_struct|.
...
\begin{enumerate}
  \item Full - all used
  \item Empty - all free
  \item Partial - some free, some used
\end{enumerate}

Upon request, slab allocator

\begin{enumerate}
  \item Uses free struct in partial slab
  \item If none, takes one from empty slab
  \item If no empty slab, create new empty
\end{enumerate}

Slab started in Solaris, now wide-spread for both kernel mode and user memory in
various OSes. Linux 2.2 had SLAB, now has both SLOB and SLUB allocators


\section{Prepaging}

To reduce the large number of page faults that occurs at process startup,
\textbf{prepaging} can be used to bring in more than just the demand page.
Prepage all or some of the pages a process will need, before they are referenced,
but if prepaged pages are unused, I/O and memory was wasted.

\paragraph{Example: } Assume \textit{s} pages are prepaged and $\alpha$ of the pages is
used

\begin{itemize}
  \item \textit{s} cost of $s \cdot \alpha$ save pages faults > or < than the cost of prepaging. $s \cdot (1 - \alpha)$ unnecessary  pages?
  \item $\alpha$ near zero $\to$ prepaging loses
\end{itemize}

\section{Page Size}

Sometimes OS designers have a choice, especially if running on custom-built CPU.
...
Always power of 2, usually in the range $2^{12}\,(4096 \text{byte})$ to $2^{22}\,(4194304 \text{byte})$.
On average, growing over time.

\section{TLB Reach}
\textbf{TLB Reach} - The amount of memory accessible from the TLB.
...
\begin{itemize}
  \item \textbf{Increase the Page Size: } This may lead to an increase in fragmentation as not all
        applications require a large page size
  \item \textbf{Provide Multiple Page Sizes: }This allows applications that
        require larger page sizes the opportunity to use them without an increase in
        fragmentation
\end{itemize}

\paragraph{Example: Program structure}

\texttt{int[128,128] data;}, Each row is stored in one page.
...
\begin{center}
  ...
  \begin{minipage}{1\linewidth}
    % Primo blocco: 50% dello spazio per il codice
    \begin{minipage}{0.65\linewidth}
      \textbf{I/O Interlock} - Pages must sometimes be locked into memory.
      Consider I/O - Pages that are used for copying
      a ﬁle from a device must be locked from being selected for eviction by a page
      replacement algorithm

      \textbf{Pinning} of pages to lock into memory.
...
maximum. Working set minimum is the minimum number of pages the
process is guaranteed to have in memory. A process may be assigned as many pages up to its working set
...
restore the amount of free memory. Working set trimming removes pages from processes that have
pages in excess of their working set minimum


\subsection{Solaris example}
...
\begin{center}
  ...
  \begin{minipage}{1\linewidth}
    ...
    \begin{minipage}{0.65\linewidth}
      ...
      Lotsfree - threshold parameter (amount of free memory) to. gin paging.
      Desfree - threshold parameter to increasing paging. Minfree - threshold
      parameter to being swapping. Paging is performed by pageout process.
      Pageout scans pages using modiﬁed clock algorithm. Scanrate is the rate at
      which pages are scanned. This ranges from slowscan to fastscan. Pageout is
...
\section{Performance of Demand Paging}
...
\begin{itemize}
  \item select for replacement one of its frames
  \item select for replacement a frame from a process with lower
        priority number
\end{itemize}

\subsection{Memory Compression}

An alternative to paging is \textbf{memory compression}.


\textbf{Memory compression} - rather than paging out modiﬁed frames to swap space, we
compress several frames into a single frame, enabling the system to reduce
...
triggers page replacement. The replacement algorithm (say, an LRU approximation
algorithm) selects four frames -- 15, 3, 35, and 26 to place on the free-frame
list. It ﬁrst places these frames on a modiﬁed-frame list. Typically, the
modiﬁed-frame list would next be written to swap space, making the frames
available to the free-frame list. An alternative strategy is to compress a
number of frames{\\mdash}say, three{\\mdash}and store their compressed versions n
a single page frame.
