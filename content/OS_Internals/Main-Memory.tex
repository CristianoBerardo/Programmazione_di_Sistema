...
\subsection{Miglioramento dell'Algoritmo della Seconda Chance}
...
\noindent\textbf{Svantaggio}: frammentazione
...
\begin{center}
  ...
  \begin{minipage}{1\linewidth}
    % Primo blocco: 50% dello spazio per il codice
    \begin{minipage}{0.65\linewidth}
      Una strategia alternativa utilizza gli \textbf{slab} - composti da una o più pagine fisicamente contigue -
...
Talvolta i progettisti di SO hanno una scelta, specialmente se eseguono su CPU custom-built.
La selezione della dimensione della pagina deve prendere in considerazione:

\begin{itemize}
  \item Frammentazione
  \item Dimensione della tabella delle pagine
  \item Overhead dell'I/O
  \item Località
  \item Risoluzione
  \item Dimensione ed efficacia del TLB
  \item Numero di page fault
\end{itemize}
...
Idealmente, il working set di ogni processo è memorizzato nel TLB, altrimenti c'è
un alto grado di page fault.


\begin{itemize}
  \item \textbf{Aumentare la Dimensione della Pagina: } Questo può portare a un aumento della frammentazione poiché non tutte
...
\texttt{int[128,128] data;} - ogni riga è memorizzata in una pagina.
