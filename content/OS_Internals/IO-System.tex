...
% more efﬁcient if non-zero infrequently?
...
%   \item Bus mastering of DMA controller - grabs bus from CPU. Cycle stealing from CPU but still much more efﬁcient
...
% \end{center}
\section{Panoramica}
(non lo abbiamo visto, abbiamo fatto cap 11 e 13)
...
\begin{flushleft}
  \begin{minipage}{0.65\linewidth}
    ...
    I dispositivi hanno indirizzi, utilizzati da: Istruzioni di I/O dirette o Memory-mapped I/O (MMIO), dati del dispositivo e registri di comando mappati nello
...
\section{Processo in sei fasi per eseguire un trasferimento DMA}

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/images/2026-03-31-17-09-14.png}
\end{figure}
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.25\linewidth]{content/OS_Internals/images/2026-03-31-17-11-50.png}
\end{figure}
...
\begin{itemize}
  ...
  \item Accesso tramite memory-mapped file possibile
        \begin{itemize}
          \item[-] File mappato nella memoria virtuale e cluster portati tramite demand paging
        \end{itemize}
  \item DMA
\end{itemize}
...
\begin{center}
  \begin{minipage}[t]{0.45\linewidth}
    \section{Tabella di stato del dispositivo (Device-status Table)}
    \centering
    \includegraphics[width=\linewidth]{./content/OS_Internals/images/2026-03-31-17-37-10.png}
  \end{minipage}%
  \hfill
  \begin{minipage}[t]{0.45\linewidth}
    \section{Dispositivi I/O comuni per PC e Data-center e velocità delle interfacce}
    \centering
    \includegraphics[width=0.5\linewidth]{./content/OS_Internals/images/2026-03-31-17-37-28.png}
  \end{minipage}
\end{center}
...
\subsubsection{Caching} - dispositivo più veloce che mantiene una copia dei dati
\begin{itemize}
...
\subsubsection{Spooling} - trattiene l'output per un dispositivo
\begin{itemize}
  \item Se il dispositivo può servire solo una richiesta alla volta
  \item es. Stampa
\end{itemize}

\subsubsection{Prenotazione del dispositivo (Device reservation)} - fornisce accesso esclusivo a un dispositivo
\begin{itemize}
...
\begin{center}
  ...
  \hfill
  \begin{minipage}[t]{0.45\linewidth}
    \section{Prestazioni I/O dello Storage (e Latenza di Rete)}
    \centering
